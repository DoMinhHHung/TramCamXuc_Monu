...
export const searchSoundCloudTracks = async (params: {
  ...
  const response = await apiClient.get<SoundCloudTrack[]>('/soundcloud/tracks/search', {
    params: {
      keyword: params.keyword,
      limit: params.limit ?? 10,
    },
  });
  return unwrap<SoundCloudTrack[]>(response.data);
};

export const isSoundCloudOwnedContent = (track: SoundCloudTrack): boolean => {
  return (track.source ?? '').toUpperCase() === 'SOUNDCLOUD';
};

export const getSoundCloudStreamUrl = async (urn: string): Promise<SoundCloudStreamResponse> => {
  const response = await apiClient.get<SoundCloudStreamResponse>('/soundcloud/stream', {
    params: { urn },
  });
  return unwrap<SoundCloudStreamResponse>(response.data);
};
