...
export const SongCard = ({ song, isActive, isPlaying, onPress, onLongPress, onMetaPress, formatDuration }: Props) => (
  <Pressable
  ...
  >
    ...
    >
      ...
      <View style={styles.listInfo}>
        <Text style={[styles.listTitle, isActive && styles.listTitleActive]} numberOfLines={1}>{song.title}</Text>
      ...
      <Pressable style={styles.listMeta} onPress={onMetaPress}>
        ...
        <Text style={[styles.listArrow, isActive && styles.listArrowActive]}>{isPlaying ? '⏸' : '▶'}</Text>
      </Pressable>
    </LinearGradient>
  </Pressable>
);
