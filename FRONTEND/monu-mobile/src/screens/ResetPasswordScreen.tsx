...
const C = { bg: '#0A090E', surface: '#13111A', border: '#2A2640', accent: '#C084FC', accentDim: '#7C3AED', text: '#F3F0FF', muted: '#7B7591', error: '#F87171', success: '#4ADE80' };
const OTP_LENGTH = 6;
...
export default function ResetPasswordScreen() {
    ...
    const pwStrength = !newPassword ? null
        : pwRegex.test(newPassword)  ? { label: 'Mạnh', color: C.success }
            : newPassword.length >= 6    ? { label: 'Trung bình', color: '#FBBF24' }
                : { label: 'Yếu', color: C.error };
    ...
    return (
        <KeyboardAvoidingView style={{ flex: 1, backgroundColor: C.bg }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
            <ScrollView contentContainerStyle={{ flexGrow: 1, padding: 24, paddingTop: 60 }} keyboardShouldPersistTaps="handled">
                ...
                <Text style={[styles.heading, { color: C.text }]}>Đặt lại mật khẩu</Text>
                <Text style={{ fontSize: 14, color: C.muted, lineHeight: 22, marginBottom: 28 }}>
                    Nhập mã OTP đã gửi đến <Text style={{ color: C.accent }}>{email}</Text> và mật khẩu mới.
                </Text>
                ...
                <View style={{ flexDirection: 'row', gap: 8, marginBottom: 10 }}>
                    {otp.map((digit, i) => (
                        <TextInput key={i} ref={r => { inputs.current[i] = r; }}
                                   ...
                                   selectionColor={C.accent} placeholder="·" placeholderTextColor={C.border} />
                    ))}
                </View>

                <Pressable onPress={handleResend} disabled={countdown > 0 || resending} style={{ alignSelf: 'flex-end', marginBottom: 20 }}>
                    <Text style={{ color: countdown > 0 ? C.muted : C.accent, fontSize: 13, fontWeight: '600' }}>
                ...
                <View style={[styles.pwWrap, { backgroundColor: C.surface, borderColor: C.border }]}>
                    <TextInput style={[styles.pwInput, { color: C.text }]} value={newPassword} onChangeText={setNewPw}
                               placeholder="••••••••" placeholderTextColor={C.muted} secureTextEntry={!showPw} autoCapitalize="none" />
                ...
                <TextInput
                    style={[styles.input, { backgroundColor: C.surface, borderColor: confirmPw && newPassword !== confirmPw ? C.error : C.border, color: C.text }]}
                    value={confirmPw} onChangeText={setConfirmPw} placeholder="••••••••"
                    placeholderTextColor={C.muted} secureTextEntry={!showPw} autoCapitalize="none" />
                {confirmPw && newPassword !== confirmPw && (
                    <Text style={{ fontSize: 12, color: C.error, marginBottom: 8 }}>Mật khẩu không khớp</Text>
                )}

                <Pressable style={[styles.btn, { backgroundColor: C.accentDim }, loading && { opacity: 0.5 }]} onPress={handleReset} disabled={loading}>
...
const styles = StyleSheet.create({
    ...
    label:    { fontSize: 12, fontWeight: '600', color: C.muted, letterSpacing: 0.8, textTransform: 'uppercase', marginBottom: 8 },
