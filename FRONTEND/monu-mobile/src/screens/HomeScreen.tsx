          ...
          <>
            <View style={styles.section}>
              ...
              <View style={styles.genreWrap}>
                {genres.map(g => (
                  <Pressable key={g.id} style={[styles.genreChip, expandedGenreId === g.id && styles.genreChipActive]} onPress={() => void toggleGenre(g.id)}>
                    <Text style={[styles.genreText, expandedGenreId === g.id && styles.genreTextActive]}>{g.name}</Text>
                  </Pressable>
                ))}
              </View>

              {expandedGenreId ? [expandedGenreId].map((id) => {
              ...
              {playlists.map((p) => (
                <Pressable key={p.id} onPress={async () => {
                  ...
                  try {
                    ...
                    Alert.alert('Lỗi thêm playlist', error?.message || 'Không thể thêm vào playlist');
                  }
