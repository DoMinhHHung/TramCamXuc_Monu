...
public interface SongRepository extends JpaRepository<Song, UUID> {
        ...
        @Query(value = """
                        ...
                        WHERE s.status = 'PUBLIC'
                          ...
                          AND (:keyword IS NULL OR s.search_vector @@ plainto_tsquery('simple', unaccent(:keyword)))
                          AND (:genreId IS NULL OR EXISTS (
                                SELECT 1 FROM song_genres sg WHERE sg.song_id = s.id AND sg.genre_id = :genreId))
                          AND (:artistId IS NULL OR s.primary_artist_id = :artistId)
                          AND (:viewerId IS NULL OR NOT EXISTS (
                                SELECT 1 FROM song_reports sr WHERE sr.song_id = s.id AND sr.reporter_id = :viewerId))
                        ORDER BY
                          CASE WHEN :keyword IS NULL THEN 0
                               ELSE ts_rank(s.search_vector, plainto_tsquery('simple', unaccent(:keyword)))
                          END DESC,
                          s.created_at DESC
                        """, nativeQuery = true)
